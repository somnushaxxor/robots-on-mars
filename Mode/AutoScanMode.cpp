#include "AutoScanMode.h"
#include "../Command/RobotCommand/MoveCommand.h"
#include "../Command/RobotCommand/ScanCommand.h"
...
AutoScanMode::AutoScanMode(Robot *robot, const size_t &iterations_number)
        : AutoMode(robot), iterations_number(iterations_number) {
}

bool AutoScanMode::find_path(std::deque<Command *> &new_commands_deque) {
    ...
    bool is_border_cell_found = false;
    std::pair<int, int> border_cell_position;
    while (!cells_deque.empty()) {
        ...
        for (auto current_direction : directions) {
            ...
            if (map.check_cell_existence(next_cell)) {
                ...
                is_border_cell_found = true;
                border_cell_position = current_cell;
                break;
            }
        }
        if (is_border_cell_found) {
            break;
        }
    }
    if (is_border_cell_found) {
        std::pair<int, int> current_cell = border_cell_position;
        new_commands_deque.push_back(new ScanCommand(robot, {}));
...
void AutoScanMode::execute_next_command() {
    Robot *robot = this->get_robot();
